...
const listFormatter = (list: ListItemInterface[]) =>
  list
    .map((item: ListItemInterface) =>
      item.selected ? item : { ...item, selected: false }
    )
    .map((item: ListItemInterface) =>
      item.disabled ? item : { ...item, disabled: false }
    );
...
const HorMultiSelect = ({
  ...
  const handleSelect = (listed: ListItemInterface): void => {
    if (listed.selected === false) {
      const deselected = selectedItems.map((item) =>
        item.value === listed.value ? { ...item, selected: false } : item
      );
    ...
    if (listed.selected) {
      setSelectedItems(
        selectedItems.map((item) =>
          item.value === listed.value ? { ...item, selected: true } : item
        )
      );
    }
  };
  ...
  return (
    ...
    >
      <p className="hor-label_non-selected">{label}</p>
      <div
        onClick={handleClick}
      ...
      >
        <p style={{ marginLeft: "12px", display: "flex" }}>
          {currentlySelected.length > 0
            ? currentlySelected
                .map((item: ListItemInterface): string => item.label)
                .join(", ")
            : label}
        </p>
        <div className="drop-down-icon">
          <Icon
            color={colorPalette.greyColors[800]}
            size="26px"
            icon={isActive ? ICON_LIST.openedEye : ICON_LIST.closedEye}
          />
        </div>
      </div>
      <HorDropDown
